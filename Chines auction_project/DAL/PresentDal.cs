...
namespace Chines_auction_project.DAL
{
    public class PresentDal : IPresentDal
    {
        ...
        public async Task<List<Present>> SearchByName(string name)
        {
            try
            {
                var present = await auctionContex.Present.Where(c => c.Name.Contains(name)).ToListAsync();
                if (present == null)
        ...
        public async Task<List<Present>> SearchByDonor(string donor)////
        {
            try
            {
                var presents = await auctionContex.Present.Include(c => c.Donor).Where(p => p.Donor.FullName.Contains(donor)).ToListAsync();
        ...
        public async Task<List<Present>> SearchByNumOfPurchases(int numOfPurchases)
        {
            ...
            var presentDetails = auctionContex.Present
                      .AsEnumerable()
